...
- простая авторизация для голосования через заголовок `X-User-Id`
...
app/
  api/        # роуты fastapi
  core/       # настройки, авторизация и обработка ошибок
...
для голосования нужен заголовок `X-User-Id`
...
почему так:
- сервису нужен простой способ понять, кто именно голосует
- для этого достаточно заголовка `X-User-Id` без отдельной auth-инфраструктуры
