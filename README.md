...
тесты используют реальный postgresql и поднимают схему только через alembic-миграции. если postgresql доступен не на `localhost:5432`, можно передать `TEST_DATABASE_ADMIN_URL`:
...
4. попробовать создать опрос с пустым вариантом ответа и убедиться, что приходит `422`
5. проголосовать за существующий вариант и проверить, что в результатах увеличился `total_votes` и счетчик нужного варианта
6. проголосовать повторно тем же `voter_id` в том же опросе и убедиться, что приходит `409` и `duplicate_vote`
7. закрыть опрос и попробовать проголосовать после закрытия и убедиться, что приходит `409` и `poll_closed`
8. создать опрос с `close_after_seconds=1`, подождать 1-2 секунды и попробовать проголосовать и убедиться, что опрос закрылся автоматически
